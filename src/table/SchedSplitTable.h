...
struct SchedSplitRow
{
    using Col = SchedSplitCol;
    ...
    explicit SchedSplitRow();
    explicit SchedSplitRow(wxSQLite3ResultSet& q);
    SchedSplitRow(const SchedSplitRow& other) = default;

    int64 id() const { return SPLITTRANSID; }
    void id(const int64 id) { SPLITTRANSID = id; }
    void to_insert_stmt(wxSQLite3Statement& stmt, int64 id) const;
    void to_update_stmt(wxSQLite3Statement& stmt) const;
    SchedSplitRow& from_select_result(wxSQLite3ResultSet& q);
    ...
    SchedSplitRow& clone_from(const SchedSplitRow& other);
    bool equals(const SchedSplitRow* other) const;
    bool operator< (const SchedSplitRow& other) const { return id() < other.id(); }
    bool operator< (const SchedSplitRow* other) const { return id() < other->id(); }
    ...
    {
        bool operator()(const SchedSplitRow& x, const SchedSplitRow& y)
        {
...
// Interface to database table BUDGETSPLITTRANSACTIONS_V1
struct SchedSplitTable : public TableBase
{
    using Row = SchedSplitRow;
...
inline SchedSplitRow::SchedSplitRow(wxSQLite3ResultSet& q)
{
    from_select_result(q);
}

inline void SchedSplitRow::to_update_stmt(wxSQLite3Statement& stmt) const
{
    to_insert_stmt(stmt, id());
}

inline SchedSplitRow& SchedSplitRow::clone_from(const SchedSplitRow& other)
{
    *this = other;
    id(-1);
    return *this;
