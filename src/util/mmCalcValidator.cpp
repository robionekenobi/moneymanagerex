...
mmCalcValidator::mmCalcValidator() :
...
void mmCalcValidator::OnChar(wxKeyEvent& event)
{
    ...
    if (str == decChar) {
        ...
        if (ind < value.Length() &&
            (ind < static_cast<size_t>(selStart) || ind >= static_cast<size_t>(selEnd))
        ) {
            // check if after last decimal point there is an operation char (+-/*)
            if (value.find('+', ind + 1) >= value.Length() &&
                value.find('-', ind + 1) >= value.Length() &&
                value.find('*', ind + 1) >= value.Length() &&
                value.find('/', ind + 1) >= value.Length()
            )
                return;
...
wxObject* mmCalcValidator::Clone() const
{
    return new mmCalcValidator();
}
